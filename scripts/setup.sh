...
# Join peers to channel - Professional implementation with retry logic
join_fabric_peers() {
    local channel_name=$1
    local max_retries=${2:-3}  # Default 3 retries
    ...
    echo "  1. peer0.org1.ibn.vn:7051"
    echo "  2. peer1.org1.ibn.vn:8051"
    echo "  3. peer2.org1.ibn.vn:9051"
    echo ""
    ...
    for peer_info in "${peers[@]}"; do
        ...
        # Check if peer is already in channel
        ...
        if [ -n "$channel_list" ]; then
            print_warning "${peer_name} is already in channel ${channel_name}"
            print_info "Skipping join for ${peer_name}"
        ...
        while [ $retry_count -lt $max_retries ]; do
            ...
            # Join channel - try with block file first, then without
            local join_output=""
            local join_status=1
            
            if [ "$has_block_file" = true ] && docker exec "${peer_name}" test -f "/root/${channel_name}.block" 2>/dev/null; then
                # Join using block file
            ...
            if [ $join_status -eq 0 ]; then
                ...
                if [ $retry_count -lt $max_retries ]; then
                    print_warning "Join failed, will retry..."
        ...
        if [ "$join_success" = false ]; then
            print_error "Failed to join ${peer_name} to channel ${channel_name} after ${max_retries} attempts"
            echo "$join_output" | tail -10
            failed_peers+=("${peer_name}")
        fi
    ...
    if [ $success_count -eq ${#peers[@]} ]; then
        print_success "All ${#peers[@]} peers joined channel ${channel_name} successfully"
        return 0
        ...
        if [ ${#failed_peers[@]} -gt 0 ]; then
            ...
            print_info "You can retry joining failed peers manually"
        fi
        return 1
    else
