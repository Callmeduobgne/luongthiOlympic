...
func (s *Service) ValidateAPIKey(ctx context.Context, apiKey string) (*models.UserInfo, error) {
	// Check for hardcoded service API key (for service-to-service communication)
	// This is the API key from docker-compose.yml environment variables
	serviceAPIKey := os.Getenv("GATEWAY_API_KEY")
	if serviceAPIKey != "" && apiKey == serviceAPIKey {
		s.logger.Info("Service API key validated (hardcoded)",
			zap.String("key_preview", apiKey[:20]+"..."),
		)
		// Return a service account user info
		return &models.UserInfo{
			ID:        "service-account",
			Email:     "service@ibn.vn",
			MSPId:     "Org1MSP",
			Role:      "admin",
			CreatedAt: time.Now(),
		}, nil
