...
SELECT id, email, password_hash, msp_id, role, is_active, created_at, updated_at FROM users WHERE username = $1 OR email = $1 LIMIT 1
`
...
func (q *Queries) GetUserByUsernameOrEmail(ctx context.Context, db DBTX, email string) (User, error) {
	row := db.QueryRow(ctx, getUserByUsernameOrEmail, email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Email,
