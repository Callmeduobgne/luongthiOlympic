...
// GetPackagesByBatch handles retrieving all packages for a specific batch
// GET /api/v1/teatrace/batches/:batchId/packages
func (h *TeaTraceHandler) GetPackagesByBatch(w http.ResponseWriter, r *http.Request) {
	batchID := chi.URLParam(r, "batchId")
	if batchID == "" {
		http.Error(w, "Batch ID is required", http.StatusBadRequest)
		return
